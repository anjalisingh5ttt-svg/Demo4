  ...
  return (
    <section className="mb-24">
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-4 gap-6 ${reverse ? "lg:flex-row-reverse" : ""}`}>
        {/* Feature Slot */}
        {(featureImage?.asset?.url || featureVideo?.asset?.url) && (
          <motion.div
          ...
          >
            {featureVideo?.asset?.url ? (
              <video
                src={featureVideo.asset.url}
                autoPlay
                muted
                loop
                playsInline
        ...
        {/* Product Grid */}
        <div className={`grid grid-cols-1 sm:grid-cols-2 gap-6 ${(featureImage?.asset?.url || featureVideo?.asset?.url) ? "lg:col-span-2" : "lg:col-span-4 lg:grid-cols-4"}`}>
          {products.map((product) => (
            <ProductCard
              ...
              image={product.mainImage?.asset?.url || "/watches/shunya-1/hero.jpg"}
